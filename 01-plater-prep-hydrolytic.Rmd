...
subtitle: "Hydrolytic extracellular enzyme assays: Automating the creation of CSV templates for use with the plater package"
author: "Hava Blair"
...
The `raw_excel_hydrolytic` subdirectory has some test data that you can use to play around with this script before you feed in your own data.  **Don't forget to delete the test files (or move them to a different folder) before you use this script for your own data.**

You can have as many sheets (tabs) as you want in each excel file.  Place all the excel files you want to process in the `raw_excel_hydrolytic` sub-directory.  Do not place additional files in this sub-directory. 
...
template_fun <- function(sheet, fluor_data){
  ...
  blnk_template_b <- read.csv("./empty_templates/blank_template_b_hydro.csv",
                              blank.lines.skip = FALSE)
...
incl_templates$plater_templates
...
test_dir <- dir("./auto_templates_hydrolytic/")

test_paths <- paste0("./auto_templates_hydrolytic/", test_dir)
